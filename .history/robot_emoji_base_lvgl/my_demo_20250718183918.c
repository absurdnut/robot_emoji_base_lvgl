...
void robot_emoji_init(robot_emoji_t* emoji, lv_obj_t* parent) {  //
  ...
  lv_obj_set_size(emoji->eye_L.eyesocket, border_width / 2, border_height / 2);
  ...
  emoji->eye_R.eyesocket =
      lv_obj_create(emoji->face);  // set the place of eye_L
  // lv_obj_update_layout(emoji->face);//需要刷新后获得属性数据,set后不会立刻更新
  lv_obj_set_size(emoji->eye_R.eyesocket, border_width / 2, border_height / 2);
  ...
  lv_obj_set_size(emoji->mouth_.mouthsocket, border_width, border_height / 2);
